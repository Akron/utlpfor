...
// Compressor32 handles zero-allocation UTL-PFOR compression and
// decompression of uint32 blocks. It owns a reusable scratch buffer
// so callers do not need to manage one.
//
// A single Compressor32 is used for both compression and decompression.
// It is not safe for concurrent use; each goroutine should create its own.
//
// Common flag combinations for [Compressor32.Compress]:
//
...
type Compressor32 struct {
	scratch []uint32
}

// NewUint32 creates a [Compressor32] with a pre-allocated scratch buffer
// for uint32 block operations. The scratch buffer is sized to support
// NoInPlace and Append without internal allocation.
func NewUint32() *Compressor32 {
	return &Compressor32{
...
func (c *Compressor32) Compress(flags Flag, dst []byte, values []uint32) ([]byte, error) {
...
// and any error.
func (c *Compressor32) Decompress(dst []uint32, src []byte) ([]uint32, int, error) {
...
func (c *Compressor32) Get(pos int, src []byte) (uint32, error) {
